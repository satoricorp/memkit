...
**Default OpenAI chat model:** **`gpt-5.2`** (override via config/env when you wire it, e.g. `MEMKIT_OPENAI_MODEL` or whatever the codebase standardizes alongside `README.md`).
...
- **OpenAI (required for non–fastembed LLM features):** `OPENAI_API_KEY`; default model **`gpt-5.2`** for chat/completions-style calls (exact env name TBD in code).
